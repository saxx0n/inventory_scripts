...
ProxMox:
 ...
 - It will create inventory-groups based on ProxMox tags, stripping out any numbers.
   - "Ubuntu24.04" will become "ubuntu" for example
 - It can be run via AAP, or from the CLI, and has the abilty to source creds either from your local env, or via passed arguemnts
   - Filters out powered-off VMs, templates, or anything you blacklist
   - Groups hosts by VM type (`qemu`, `lxc`) and Proxmox tag
   - Automatically injects group_vars based on tag name (when specified group_var is defined)
   - Optionally resolves `ansible_host` via DNS
   - Supports AAP’s expected `--list` mode 
